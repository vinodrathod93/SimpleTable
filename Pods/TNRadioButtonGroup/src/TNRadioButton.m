...
- (void)createLabel {
    ...
    if (floor(NSFoundationVersionNumber) <= NSFoundationVersionNumber_iOS_6_1) {
        ...
        labelSize = [self.data.labelText sizeWithFont:self.data.labelFont forWidth:width lineBreakMode:NSLineBreakByWordWrapping];
        
    } else {
        CGRect labelRect = [self.data.labelText boundingRectWithSize:CGSizeMake(width, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.data.labelFont} context:nil];
